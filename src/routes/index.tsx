    ...
    <main className="fixed inset-0 overflow-hidden bg-background text-foreground">
      ...
      {started && (
        <button
          ...
          className="glass-strong fixed bottom-6 right-6 z-40 flex h-12 w-12 items-center justify-center rounded-full text-blue-100 hover:bg-blue-500/20 transition"
        >
          {muted ? (
      ...
      {!started ? (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="absolute inset-0 flex flex-col items-center justify-center text-center px-6 bg-gradient-to-b from-[#0a141a] to-[#060a10]"
        >
          {/* ambient glow */}
          <motion.div
            className="absolute h-[60vw] w-[60vw] max-h-[420px] max-w-[420px] rounded-full"
            style={{ background: "radial-gradient(circle, rgba(0,191,255,0.35), transparent 65%)" }}
          ...
          <h1
            className="relative z-10 font-display text-[2.6rem] leading-[1.05] sm:text-5xl md:text-7xl text-blue-100 px-2"
            style={{ textShadow: "0 0 30px rgba(0,191,255,0.8)" }}
          >
            A surprise for Ajebo
          </h1>
          <p className="relative z-10 mt-4 max-w-xs sm:max-w-md text-sm sm:text-base text-blue-200/70">
          ...
          >
            ...
            {[0, 1, 2].map((i) => (
              <motion.span
                key={i}
                className="absolute top-0 h-28 w-28 sm:h-32 sm:w-32 rounded-full border border-blue-400/40"
            ...
            >
              <svg viewBox="0 0 100 100" className="absolute inset-0 h-full w-full drop-shadow-[0_0_30px_rgba(0,191,255,0.9)]">
                <defs>
                  <radialGradient id="startHeart" cx="50%" cy="40%">
                    <stop offset="0%" stopColor="#b6e5ff" />
            ...
            <motion.span
              ...
              className="mt-6 text-[10px] uppercase tracking-[0.45em] text-blue-200/80"
            >
              💙 begin 💙
            </motion.span>
          </motion.button>
            ...
            {SCENES.map((_, i) => (
              <span
                ...
                className={`h-1.5 rounded-full transition-all duration-700 ${
                  i === sceneIndex ? "w-8 bg-blue-300" : "w-1.5 bg-blue-100/30"
                }`}
              />
            ))}
            {isLast && (
              <button
                onClick={restart}
                className="ml-4 glass-strong rounded-full px-4 py-1.5 text-[10px] uppercase tracking-[0.3em] text-blue-100 hover:bg-blue-500/10 transition"
              >
                ↻ Replay
              </button>
            )}
